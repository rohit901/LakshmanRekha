 ...
 */
public class LeftFaceGraphic extends GraphicOverlay.Graphic {
    ...
    LeftFaceGraphic(GraphicOverlay overlay, Face face, ImageCapture captureUseCase, ExecutorService executorService, Context context) {
        super(overlay);
        handler = new Handler();
        this.face = face;
        ...
        intent = new Intent(context, AfterCaptureActivity.class);
...
//                idPaints[colorID]);
        if( (bottom < ovBot && top > ovTop) && !isDetected){
            isDetected = true;
            Log.d("Leftang901","X: "+angX+", Y: "+angY+", Z: "+angZ);
            if( (angY > 36 && angY < 42) && (angX > -6 && angX < -1) && (angZ > -8 && angZ < 9) ) {
...
//                }
                ...
                File photoFile = new File(storageDir, "left_"+new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss", Locale.US)
